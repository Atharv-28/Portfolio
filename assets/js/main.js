...
tl.fromTo(
  ".text",
  { opacity: 0 },
  { opacity: 1, duration: 0.85, stagger: 0.25 }
);
tl.to(".slider", { y: "-100%", duration: 0.75, delay: 0 });
//tl.to(".intro", { y: "-100%", duration: .5 }, "-=.5");
tl.to("html", { overflowY: "visible", delay: -0.2 });
tl.fromTo("#home", { opacity: 0 }, { opacity: 1, duration: 0.4, delay: -0.36 });
tl.fromTo("nav", { opacity: 0 }, { opacity: 1, duration: 0.4, delay: -0.29 });
...
function redirect(project) {
  ...
  tl2.set(".intro", { y: "0%" });
  tl2.to(".intro", { opacity: 1, duration: 0.5 });
  setTimeout(function () {
...
function scrollActive() {
  ...
  sections.forEach((current) => {
    ...
    if (scrollY > sectionTop && scrollY <= sectionTop + sectionHeight) {
      ...
      if (sectionId === "contact" && activarconfetti === true) {
        /* START CONFETTI JS*/
        confetti.start();
        setTimeout(function () {
          confetti.stop();
          activarconfetti = false;
        }, 3000); /* STOP CONFETTI JS*/
      }
    } else {
      document
        .querySelector(".nav__menu a[href*=" + sectionId + "]")
